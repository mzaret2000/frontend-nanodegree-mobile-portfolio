...
var resizePizzas = function(size) {
  ...
  //move vars out of for loop change dom query type getElementsByClassName
  ...
  function changePizzaSizes(size) {
    ...
    for (i; i < randomPizzaContainer.length; i++) {
...
function updatePositions() {
  ...
  //calculate phase before for loop
  var p = 0;
  var phase2 = [];
  var scrollTop = document.body.scrollTop;
  for (p; p <= 4; p++) {
    phase2[p] = Math.sin((scrollTop / 1250) + (p%5));
  ...
  for (i; i < items.length; i++) {
    ...
    //console.log(phase);
    //console.log(phase2[i%5])
    //items[i].style.left = items[i].basicLeft + 100 * phase2[i%5] + 'px';
    var posish = items[i].basicLeft + 100 * phase2[i%5] - screen.width/2;
    items[i].style.transform = 'translateX(' + posish + 'px)';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //reduce to 50 pizzas. 200 is more than users sees in window.
  for (var i = 0; i < 50; i++) {
